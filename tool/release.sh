#!/usr/bin/env bash
# Builds, signs and ships Gozdar for macOS and iOS.
#
#   ./tool/release.sh macos            # → dist/gozdar-<version>-macos.zip, notarized
...
open_keychain() {
  [ -n "$KEYCHAIN" ] && return 0
  KEYCHAIN="gozdar-release.keychain"
  local pw; pw="$(uuidgen)"
  security delete-keychain "$KEYCHAIN" 2>/dev/null || true
...
WORK="$(mktemp -d -t gozdar-release)"
...
build_macos() {
  ...
  mkdir -p "$DIST"
  local zip="$DIST/gozdar-v$VERSION-macos.zip"
  # The release asset is a zip of the .app, the shape CI has always attached.
  # Apple notarizes a zip happily, but the ticket staples to the .app inside
  # it — so the zip is made twice: once to submit, once for real.
  rm -f "$zip"
...
build_ios() {
  ...
  if [ -n "${IOS_PROFILE:-}" ]; then
    ...
    mkdir -p "$dir"
    printf '%s' "$IOS_PROFILE" | base64 --decode > "$dir/gozdar-release.mobileprovision"
